...
async function ensureData() {
  ...
  // Restore last selected origin from localStorage if valid
  const saved = (typeof window !== 'undefined') ? (localStorage.getItem('tg.distance.originId') || '') : '';
  if (originId.value && cityStore.cities[originId.value]) {
    // keep current
  } else if (saved && cityStore.cities[saved]) {
    originId.value = saved;
  } else {
...
watch(originId, (val) => {
  try { if (val) localStorage.setItem('tg.distance.originId', val); } catch {}
  hydrateFromEdges();
});

// IDs are generated by the database in backend mode; no client-side ID generation is needed.
...
async function onAddCity() {
  ...
  const newId = await cityService.add({ id: '', name: newCityName.value, buyableProductIds: products });
  cityStore.cities = await cityService.list();
  originId.value = newId;
    newCityName.value = '';
    hydrateFromEdges();
    ui.success('城市已添加');
  } catch (e: any) {
